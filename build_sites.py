...
__ROWS__
  ...
  function render() {
    ...
    for (const b of list) {
      ...
      a.className = "card"; a.href = safeUrl(b.url); a.target = "_blank"; a.rel = "noopener";
      a.innerHTML = `
        <div class="card-top"><span class="brand">${esc(b.brand)}</span><span class="price">${kr(b.price)}</span></div>
        <div class="title">${esc(b.title)}</div>
        <div class="meta">${b.size ? `<span class="size">Storlek ${esc(b.size)}</span>` : ``}<span class="loc">${esc(b.loc)}</span></div>
